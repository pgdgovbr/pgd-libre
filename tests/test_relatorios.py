...
async def _setup_base(db: AsyncSession, admin, cod_ua: int = 90001):
    ua = await criar_unidade_autorizadora(
        ...
        nome="UA Rel", sigla="UAR", user=admin,
    )
    ...
    ui = await criar_unidade_instituidora(
        db, unidade_autorizadora_id=ua.id, cod_unidade_instituidora=90010,
        nome="UI Rel", sigla="UIR", ato_instituicao_ref="P1",
        data_instituicao=date(2024, 1, 1), tipos_atividades="T",
        conteudo_minimo_tcr="C", prazo_antecedencia_convocacao_dias=5, user=admin,
    )
    ue = UnidadeExecucao(
        unidade_instituidora_id=ui.id, cod_unidade_executora=90020,
        nome="UE Rel", sigla="UER",
    )
    db.add(ue)
    await db.commit()
    await db.refresh(ue)
...
async def test_relatorio_sem_plano_trabalho_exclui_com_plano_ativo(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db, id_plano_entregas="PE-R2", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90002, cod_unidade_instituidora=90010,
        cod_unidade_executora=90020, unidade_execucao_id=ue.id,
        data_inicio=date(2026, 1, 1), data_termino=date(2026, 12, 31), user=admin,
    )
    await criar_entrega(
        db, id_entrega="E-R2", plano_entregas_id=pe.id, nome_entrega="E",
    ...
    pt = await criar_plano_trabalho(
        db, id_plano_trabalho="PT-R2", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90002, cod_unidade_executora=90020,
        cod_unidade_lotacao_participante=90020, participante_id=p.id,
...
async def test_relatorio_registros_atraso(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db, id_plano_entregas="PE-R3", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90003, cod_unidade_instituidora=90010,
        cod_unidade_executora=90020, unidade_execucao_id=ue.id,
        data_inicio=date(2025, 1, 1), data_termino=date(2025, 12, 31), user=admin,
    )
    await criar_entrega(
        db, id_entrega="E-R3", plano_entregas_id=pe.id, nome_entrega="E",
        meta_entrega=1, tipo_meta=TipoMeta.UNIDADE, data_entrega=date(2025, 12, 31),
        nome_unidade_demandante="D", nome_unidade_destinataria="D", user=admin,
    )
    pt = await criar_plano_trabalho(
        db, id_plano_trabalho="PT-R3", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90003, cod_unidade_executora=90020,
        cod_unidade_lotacao_participante=90020, participante_id=p.id,
        cpf_participante=p.cpf, matricula_siape=p.matricula_siape, tcr_id=tcr.id,
        data_inicio=date(2025, 1, 1), data_termino=date(2025, 12, 31),
        carga_horaria_disponivel=2000, criterios_avaliacao="CA",
        plano_entregas_id=pe.id, user=admin,
    )
    await adicionar_contribuicao(
    ...
    resultado = await relatorio_registros_atraso(db, referencia=referencia, cod_unidade_autorizadora=90003)
...
async def test_relatorio_avaliacoes_pendentes(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db, id_plano_entregas="PE-R4", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90004, cod_unidade_instituidora=90010,
        cod_unidade_executora=90020, unidade_execucao_id=ue.id,
        data_inicio=date(2025, 1, 1), data_termino=date(2025, 12, 31), user=admin,
    )
    await criar_entrega(
        db, id_entrega="E-R4", plano_entregas_id=pe.id, nome_entrega="E",
        meta_entrega=1, tipo_meta=TipoMeta.UNIDADE, data_entrega=date(2025, 12, 31),
        nome_unidade_demandante="D", nome_unidade_destinataria="D", user=admin,
    )
    pt = await criar_plano_trabalho(
        db, id_plano_trabalho="PT-R4", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90004, cod_unidade_executora=90020,
        cod_unidade_lotacao_participante=90020, participante_id=p.id,
        cpf_participante=p.cpf, matricula_siape=p.matricula_siape, tcr_id=tcr.id,
        data_inicio=date(2025, 1, 1), data_termino=date(2025, 12, 31),
        carga_horaria_disponivel=2000, criterios_avaliacao="CA",
    ...
    avaliacao = AvaliacaoRegistrosExecucao(
        ...
        data_inicio_periodo_avaliativo=date(2025, 1, 1),
        data_fim_periodo_avaliativo=date(2025, 1, 31),
        data_registro_participante=datetime(2025, 2, 5, tzinfo=timezone.utc),
    ...
    referencia = date(2026, 3, 1)
    resultado = await relatorio_avaliacoes_pendentes(db, referencia=referencia, cod_unidade_autorizadora=90004)
    assert len(resultado) >= 1
    assert any(r.id == avaliacao.id for r in resultado)
...
async def test_relatorio_pe_avaliacao_pendente(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db, id_plano_entregas="PE-R9", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90006, cod_unidade_instituidora=90010,
        cod_unidade_executora=90020, unidade_execucao_id=ue.id,
        data_inicio=date(2025, 1, 1), data_termino=date(2025, 6, 30), user=admin,
    )
    await criar_entrega(
        db, id_entrega="E-R9", plano_entregas_id=pe.id, nome_entrega="E",
        meta_entrega=1, tipo_meta=TipoMeta.UNIDADE, data_entrega=date(2025, 6, 30),
        nome_unidade_demandante="D", nome_unidade_destinataria="D", user=admin,
    )
...
async def test_relatorio_pe_avaliacao_pendente_exclui_avaliado(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db, id_plano_entregas="PE-R9B", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=90007, cod_unidade_instituidora=90010,
        cod_unidade_executora=90020, unidade_execucao_id=ue.id,
        data_inicio=date(2025, 1, 1), data_termino=date(2025, 6, 30), user=admin,
    )
    await criar_entrega(
        db, id_entrega="E-R9B", plano_entregas_id=pe.id, nome_entrega="E",
        meta_entrega=1, tipo_meta=TipoMeta.UNIDADE, data_entrega=date(2025, 6, 30),
        nome_unidade_demandante="D", nome_unidade_destinataria="D", user=admin,
    )
    pe.status = STATUS_PE_CONCLUIDO
