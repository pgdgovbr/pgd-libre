...
async def _setup_for_notif(db):
    ...
    from src.services.plano_trabalho import criar_plano_trabalho, iniciar_execucao_pt, registrar_execucao
    ...
    ua = await criar_unidade_autorizadora(
        db, cod_unidade_autorizadora=1, origem_unidade=OrigemUnidade.SIAPE,
        nome="UA N", sigla="UAN", user=admin,
    )
    await criar_ato_autorizacao(
        db, unidade_autorizadora_id=ua.id, autoridade="A",
        data_publicacao=date(2024, 1, 1), referencia="R", user=admin,
    )
    ui = await criar_unidade_instituidora(
        db, unidade_autorizadora_id=ua.id, cod_unidade_instituidora=100,
        nome="UI N", sigla="UIN", ato_instituicao_ref="R2",
        data_instituicao=date(2024, 1, 10), tipos_atividades="T",
        modalidades_autorizadas=[1], conteudo_minimo_tcr="C",
        prazo_antecedencia_convocacao_dias=5, user=admin,
    )
    ue = UnidadeExecucao(unidade_instituidora_id=ui.id, cod_unidade_executora=200, nome="UE N", sigla="UEN")
    db.add(ue)
    await db.commit()
    await db.refresh(ue)

    p = await cadastrar_participante(
        db, origem_unidade=OrigemUnidade.SIAPE, cod_unidade_autorizadora=ua.cod_unidade_autorizadora,
        cod_unidade_lotacao=ue.cod_unidade_executora, matricula_siape="1234567",
        cod_unidade_instituidora=ui.cod_unidade_instituidora, cpf="11144477735",
        nome="Notif Test", email="notif@t.com", modalidade_execucao=1,
        data_assinatura_tcr=date(2024, 3, 1), tipo_vinculo=TipoVinculo.EFETIVO,
        unidade_execucao_id=ue.id, user=admin,
    )
    tcr = await pactu_tcr(
        db, participante_id=p.id, chefia_user_id=admin.id, modalidade_execucao=1,
        regime_execucao=RegimeExecucao.INTEGRAL, prazo_antecedencia_convocacao_dias=5,
        canais_comunicacao=["email"], responsabilidades="R",
        ciencia_instalacoes_ergonomia=True, ciencia_nao_direito_adquirido=True,
        ciencia_custeio_estrutura=True, user=admin,
    )
    await assinar_tcr_chefia(db, tcr_id=tcr.id, user=admin)
    pt = await criar_plano_trabalho(
        db, id_plano_trabalho="PT-N", origem_unidade=OrigemUnidade.SIAPE,
        ...
        participante_id=p.id, cpf_participante=p.cpf, matricula_siape=p.matricula_siape,
        data_inicio=date(2024, 3, 1), data_termino=date(2024, 12, 31),
        carga_horaria_disponivel=160, criterios_avaliacao="C", user=admin,
    )
    pt = await iniciar_execucao_pt(db, plano_id=pt.id, user=admin)
    are = await registrar_execucao(
        db, id_periodo_avaliativo="P-N", plano_trabalho_id=pt.id,
        data_inicio_periodo_avaliativo=date(2024, 3, 1),
        data_fim_periodo_avaliativo=date(2024, 3, 31),
        descricao_execucao="Exec", user=admin,
    )
    return are, admin
...
async def test_avaliacao_realizada_cria_notificacao(db: AsyncSession):
    ...
    await avaliar_registros_execucao(
        db, avaliacao_id=are.id, nota=3, data_avaliacao=date(2024, 4, 10), user=admin,
    )
...
async def test_plano_aprovado_cria_notificacao(db: AsyncSession):
    ...
    ua = await criar_unidade_autorizadora(
        db, cod_unidade_autorizadora=2, origem_unidade=OrigemUnidade.SIAPE,
        nome="UA N2", sigla="UN2", user=admin,
    )
    await criar_ato_autorizacao(
        db, unidade_autorizadora_id=ua.id, autoridade="A",
        data_publicacao=date(2024, 1, 1), referencia="R", user=admin,
    ...
    ui = await criar_unidade_instituidora(
        db, unidade_autorizadora_id=ua.id, cod_unidade_instituidora=200,
        nome="UI N2", sigla="UN2", ato_instituicao_ref="R2",
        data_instituicao=date(2024, 1, 10), tipos_atividades="T",
        modalidades_autorizadas=[1], conteudo_minimo_tcr="C",
        prazo_antecedencia_convocacao_dias=5, user=admin,
    )
    ue = UnidadeExecucao(unidade_instituidora_id=ui.id, cod_unidade_executora=300, nome="UE N2", sigla="UN2")
    db.add(ue)
    await db.commit()
    await db.refresh(ue)
    ...
    p = await cadastrar_participante(
        ...
        cod_unidade_lotacao=ue.cod_unidade_executora, matricula_siape="1234567",
        cod_unidade_instituidora=ui.cod_unidade_instituidora, cpf="11144477735",
        nome="Notif2", email="notif2@t.com", modalidade_execucao=1,
        data_assinatura_tcr=date(2024, 3, 1), tipo_vinculo=TipoVinculo.EFETIVO,
        unidade_execucao_id=ue.id, user=admin,
    )
    tcr = await pactu_tcr(
    ...
    await criar_plano_trabalho(
        db, id_plano_trabalho="PT-NOTIF2", origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=ua.cod_unidade_autorizadora,
        cod_unidade_executora=ue.cod_unidade_executora,
        cod_unidade_lotacao_participante=ue.cod_unidade_executora,
        participante_id=p.id, cpf_participante=p.cpf, matricula_siape=p.matricula_siape,
