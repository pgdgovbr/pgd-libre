...
async def _setup(db: AsyncSession, admin, cod_ua: int = 130001):
    ...
    ui = await criar_unidade_instituidora(
        db,
        unidade_autorizadora_id=ua.id,
        cod_unidade_instituidora=130010,
        nome="UI AFA",
        sigla="UIF",
        ato_instituicao_ref="AFA",
...
async def _make_participante(
    db, admin, ue, ua, ui, matricula="1300001", cod_ua=130001, modalidade=1
):
...
async def test_registrar_afastamento_data_fim_anterior_inicio_rejeitado(
    db: AsyncSession,
):
...
async def test_relatorio_afastamentos_destaca_afastados_no_periodo(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db,
        id_plano_entregas="PE-AFA-1",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=130010,
        cod_unidade_instituidora=130010,
        cod_unidade_executora=130020,
        unidade_execucao_id=ue.id,
    ...
    for i, mat in enumerate(["1300010", "1300011", "1300012"], start=1):
        ...
        pt = await criar_plano_trabalho(
            db,
            id_plano_trabalho=f"PT-AFA-{i}",
            origem_unidade=OrigemUnidade.SIAPE,
            cod_unidade_autorizadora=130010,
            cod_unidade_executora=130020,
            cod_unidade_lotacao_participante=130020,
            participante_id=p.id,
    ...
    afastados = await relatorio_afastamentos(db, cod_unidade_autorizadora=130010, ano=2026, mes=2)
...
async def test_relatorio_afastamentos_fora_periodo_excluido(db: AsyncSession):
    ...
    afastados = await relatorio_afastamentos(db, cod_unidade_autorizadora=130011, ano=2026, mes=3)
    assert afastados == []
...
async def test_relatorio_afastamentos_em_curso_aparece(db: AsyncSession):
    ...
    afastados_jan = await relatorio_afastamentos(
        db, cod_unidade_autorizadora=130012, ano=2026, mes=1
    )
    afastados_mar = await relatorio_afastamentos(
        db, cod_unidade_autorizadora=130012, ano=2026, mes=3
    )
