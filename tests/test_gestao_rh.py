...
from src.services.avaliacao import (
    mapear_avaliacao_customizada,
    validate_escala_customizada,
)
...
async def _setup(db: AsyncSession, admin, cod_ua: int = 110001):
    ...
    ui = await criar_unidade_instituidora(
        db,
        unidade_autorizadora_id=ua.id,
        cod_unidade_instituidora=110010,
        nome="UI RH",
        sigla="UIH",
...
async def _make_participante(
    db, admin, ue, ua, ui, matricula="1110001", cod_ua=110001, modalidade=1
):
...
async def test_equipamentos_autorizado_para_tt_integral(db: AsyncSession):
    ...
    p = await _make_participante(
        db, admin, ue, ua, ui, matricula="1110002", cod_ua=110002, modalidade=3
    )
...
async def test_declaracao_acumulacao_persistida(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db,
        id_plano_entregas="PE-RH29",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=110003,
        cod_unidade_instituidora=110010,
        cod_unidade_executora=110020,
        unidade_execucao_id=ue.id,
    ...
    pt = await criar_plano_trabalho(
        db,
        id_plano_trabalho="PT-RH29",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=110003,
        cod_unidade_executora=110020,
        cod_unidade_lotacao_participante=110020,
        participante_id=p.id,
...
async def test_contribuicao_acao_desenvolvimento_persiste_rotulo(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db,
        id_plano_entregas="PE-RH101",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=110004,
        cod_unidade_instituidora=110010,
        cod_unidade_executora=110020,
        unidade_execucao_id=ue.id,
        data_inicio=date(2026, 1, 1),
...
async def test_relatorio_frequencia_retorna_participantes_no_periodo(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db,
        id_plano_entregas="PE-FREQ-1",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=120001,
        cod_unidade_instituidora=110010,
        cod_unidade_executora=110020,
        unidade_execucao_id=ue.id,
        data_inicio=date(2026, 1, 1),
