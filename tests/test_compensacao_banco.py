...
from src.models.plano import AvaliacaoRegistrosExecucao
...
async def _make_setup(db: AsyncSession, admin, cod_ua: int = 80001):
    ...
    ui = await criar_unidade_instituidora(
        db,
        unidade_autorizadora_id=ua.id,
        cod_unidade_instituidora=80010,
        nome="UI CB",
        sigla="UCB",
...
async def test_get_carga_compensacao_pendente(db: AsyncSession):
    ...
    pe = await criar_plano_entregas(
        db,
        id_plano_entregas="PE-CB-001",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=80001,
        cod_unidade_instituidora=80010,
        cod_unidade_executora=80020,
        unidade_execucao_id=ue.id,
    ...
    pt = await criar_plano_trabalho(
        db,
        id_plano_trabalho="PT-CB-001",
        origem_unidade=OrigemUnidade.SIAPE,
        cod_unidade_autorizadora=80001,
        cod_unidade_executora=80020,
        cod_unidade_lotacao_participante=80020,
        participante_id=p.id,
